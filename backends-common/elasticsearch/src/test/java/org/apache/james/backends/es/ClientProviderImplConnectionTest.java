...
public class ClientProviderImplConnectionTest {
    ...
    public void connectingASingleServerShouldWork() {
        ElasticSearchConfiguration configuration = ElasticSearchConfiguration.builder()
            .addHost(Host.from(es1.getContainerIp(), ES_APPLICATIVE_PORT)).build();
        Awaitility.await()
    ...
    public void connectingAClusterShouldWork() {
        ElasticSearchConfiguration configuration = ElasticSearchConfiguration.builder()
            ...
            .addHost(Host.from(es2.getContainerIp(), ES_APPLICATIVE_PORT)).build();
        Awaitility.await()
            .atMost(1, TimeUnit.MINUTES)
            .pollInterval(5, TimeUnit.SECONDS)
            .until(() -> isConnected(ClientProviderImpl.fromConfiguration(configuration)));
    ...
    public void connectingAClusterWithAFailedNodeShouldWork() {
        ...
        ElasticSearchConfiguration configuration = ElasticSearchConfiguration.builder()
            ...
            .addHost(Host.from(es2Ip, ES_APPLICATIVE_PORT)).build();
        Awaitility.await()
            .atMost(1, TimeUnit.MINUTES)
            .pollInterval(5, TimeUnit.SECONDS)
            .until(() -> isConnected(ClientProviderImpl.fromConfiguration(configuration)));
