...
public class SetMailboxesMethodStepdefs {
    ...
    public void mailboxWithMessages(String mailboxName, int messageCount) throws Throwable {
        mainStepdefs.jmapServer.getProbe(PojoMailboxProbe.class).createMailbox("#private", userStepdefs.lastConnectedUser, mailboxName);
    ...
    private void appendMessage(MailboxPath mailboxPath, int i) throws MailboxException {
        String content = "Subject: test" + i + "\r\n\r\n"
                + "testBody" + i;
        mainStepdefs.jmapServer.getProbe(PojoMailboxProbe.class).appendMessage(userStepdefs.lastConnectedUser, mailboxPath,
    ...
    public void renamingMailbox(String actualMailboxName, String newMailboxName) throws Throwable {
        ...
        Mailbox mailbox = mainStepdefs.jmapServer.getProbe(PojoMailboxProbe.class).getMailbox("#private", userStepdefs.lastConnectedUser, actualMailboxName);
    ...
    public void movingMailbox(String actualMailboxPath, String newParentMailboxPath) throws Throwable {
        String username = userStepdefs.lastConnectedUser;
        Mailbox mailbox = mainStepdefs.jmapServer.getProbe(PojoMailboxProbe.class).getMailbox("#private", username, actualMailboxPath);
        String mailboxId = mailbox.getMailboxId().serialize();
        Mailbox parent = mainStepdefs.jmapServer.getProbe(PojoMailboxProbe.class).getMailbox("#private", username, newParentMailboxPath);
    ...
    public void mailboxContainsMessages(String mailboxName, int messageCount) throws Throwable {
        String username = userStepdefs.lastConnectedUser;
        Mailbox mailbox = mainStepdefs.jmapServer.getProbe(PojoMailboxProbe.class).getMailbox("#private", username, mailboxName);
        String mailboxId = mailbox.getMailboxId().serialize();
