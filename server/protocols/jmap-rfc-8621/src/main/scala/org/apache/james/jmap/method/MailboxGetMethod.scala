...
class MailboxGetMethod @Inject() (serializer: Serializer,
  ...
  private def getMailboxes(mailboxGetRequest: MailboxGetRequest, mailboxSession: MailboxSession): SFlux[Mailbox] = mailboxGetRequest.ids match {
    ...
    case _ => SFlux.raiseError(new IllegalArgumentException("Getting mailboxes by Ids is not supported yet"))
  }

  private def getAllMailboxes(mailboxSession: MailboxSession): SFlux[Mailbox] = {
    val userMailboxesMono: SMono[Seq[MailboxMetaData]] = getAllMailboxesMetaData(mailboxSession).collectSeq()
    val quotaLoaderMono: SMono[QuotaLoaderWithPreloadedDefault] =
      SMono.fromCallable(() => quotaFactory.loadFor(mailboxSession))
        .subscribeOn(Schedulers.elastic)

    quotaLoaderMono.flatMapMany(quotaLoader =>
      userMailboxesMono.flatMapMany(mailboxesMetaData =>
  ...
  private def getAllMailboxesMetaData(mailboxSession: MailboxSession): SFlux[MailboxMetaData] =
