...
class QuotaLoaderWithPreloadedDefaultFactory @Inject()(quotaRootResolver: UserQuotaRootResolver, quotaReader: QuotaReader) {
  ...
  def loadFor(session: MailboxSession): QuotaLoaderWithPreloadedDefault = new QuotaLoaderWithPreloadedDefault(quotaRootResolver,
    quotaReader,
    session,
    getUserDefaultQuotas(session))

  @throws[MailboxException]
  private def getUserDefaultQuotas(session:MailboxSession): Quotas = {
...
class QuotaLoaderWithPreloadedDefault(quotaRootResolver: UserQuotaRootResolver,
  ...
  @throws[MailboxException]
  override def getQuotas(mailboxPath: MailboxPath): Quotas =
