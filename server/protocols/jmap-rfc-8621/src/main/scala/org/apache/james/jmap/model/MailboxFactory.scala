...
class MailboxFactory @Inject() (subscriptionManager: SubscriptionManager) {
  ...
  def create(mailboxMetaData: MailboxMetaData,
             ...
             quotaLoader: QuotaLoader): Mailbox = {

    val id: MailboxId = mailboxMetaData.getId

    val name: MailboxName = Refined.unsafeApply(mailboxMetaData.getPath
      .getName
    ...
    val quotas: Quotas = quotaLoader.getQuotas(mailboxMetaData.getPath)
    ...
    val unreadEmails: UnreadEmails = UnreadEmails(Refined.unsafeApply(sanitizedCounters.getUnseen))
    val unreadThreads: UnreadThreads = UnreadThreads(Refined.unsafeApply(sanitizedCounters.getUnseen))
    val totalEmails: TotalEmails = TotalEmails(Refined.unsafeApply(sanitizedCounters.getCount))
    val totalThreads: TotalThreads = TotalThreads(Refined.unsafeApply(sanitizedCounters.getCount))
